...
function testUserAgent_has(test, pass, miss) {
    var ua = UserAgent.parse("Mozilla/5.0 (iPhone; CPU iPhone OS 8_0 like Mac OS X) AppleWebKit/536.26 (KHTML, like Gecko) Version/6.0 Mobile/10A403 Safari/8536.25");

    if (ua.has("iPhone") && ua.get("iPhone") === true) {
        test.done(pass());
    } else {
        test.done(miss());
    }
...
function testUserAgent_DISABLE_CACHE(test, pass, miss) {
    ...
    if (ua2.BASE_BROWSER === "Edge" && ua2.Edge) {
        test.done(pass());
    } else {
        test.done(miss());
    }
