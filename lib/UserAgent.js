...
function UserAgent(userAgent, // @arg String = navigator.userAgent
                   options) { // @arg Object = {} - { FULL_SCREEN, FILE_SYSTEM, DISPLAY_DPR, DISPLAY_LONG, DISPLAY_SHORT }
...
function _detectDevice(ua, os, osVersion, options) {
    ...
    var screenHeight  = screen["width"]  || 0;
    var DISPLAY_DPR   = options["DISPLAY_DPR"]   || global["devicePixelRatio"] || 1.0;
    var DISPLAY_LONG  = options["DISPLAY_LONG"]  || Math.max(screenWidth, screenHeight);
    var DISPLAY_SHORT = options["DISPLAY_SHORT"] || Math.min(screenWidth, screenHeight);
    var retina        = DISPLAY_DPR >= 2;
    var longEdge      = Math.max(DISPLAY_LONG, DISPLAY_SHORT); // iPhone 4s: 480, iPhone 5: 568
